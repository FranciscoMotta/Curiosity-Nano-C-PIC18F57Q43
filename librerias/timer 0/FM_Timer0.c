...
void FM_Timer0_Init (_timer0_params_config_t *timer_configs)
{
    ...
    T0CON0 |= (timer_configs->timer0_postecaler << _T0CON0_OUTPS0_POSITION); // Postescaler
    
    /* Registro T0CON1 */
    T0CON1 |= (timer_configs->timer0_clock_source << _T0CON1_CS0_POSITION); // Clock source
    T0CON1 |= (timer_configs->timer0_counter_sync << _T0CON1_ASYNC_POSITION); // Sync
    T0CON1 |= (timer_configs->timer0_prescaler << _T0CON1_T0CKPS0_POSITION); // Prescaler
}
